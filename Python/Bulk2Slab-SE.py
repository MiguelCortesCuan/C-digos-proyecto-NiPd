...
# ---------------- USER SETTINGS ----------------
ROOT_DIR = Path(r"G:\My Drive\Work\UNAM\Doctorado\Proyecto\Resultados\Nanoparticles\QE\CeldaUnitaria")
DEST_DIR = Path(r"G:\My Drive\Work\UNAM\Doctorado\Proyecto\Resultados\Nanoparticles\QE\Supercell\SurfaceEnergy\111_2x2")

HKL_INPUT          = (1, 1, 1)
SCAN_FAMILY        = True
SURFACE_LABEL      = "111"

N_SUBLAYERS_TARGET = 12
VACUUM_ANGSTROM    = 11
FREEZE_N_MIDDLE    = 2              # ahora congela las capas centrales, no las de abajo
ORIGIN_SAMPLES     = 10
AREA_DECIMALS      = 3
...
OUTDIR_QE          = "."
PSEUDO_DIR         = "/tmpu/isholek_g/isholek/MIGUEL/PP/PAW"
...
NOINV_QE           = True
ASSUME_ISO_QE      = "2D"
CONV_THR_ELEC      = "1.0d-08"
FORC_CONV_THR      = "7.78d-4"
ION_DYN_QE         = "bfgs"
START_MAG_MAP      = {"Ni": 0.65, "Pd": 0.10}
...
def _parse_positions_block(lines: List[str], i: int, cell: np.ndarray):
    ...
    while j < len(lines):
        s = lines[j].strip()
        if (not s) or any(s.startswith(t) for t in _STOP_TOKENS):
            break
        t = s.split()
        if len(t) < 4:
            break
...
def group_sublayers(z: np.ndarray):
    ...
    zs = z[order]
    if len(zs) <= 1:
        return [order.tolist()], np.array([zs.mean() if len(zs) else 0.0])
    dz  = np.diff(zs)
    dzp = dz[dz > 1e-8]
    ...
    for i in range(1, len(zs)):
        if (zs[i]-zs[i-1]) <= tol:
            planes[-1].append(order[i])
        else:
            planes.append([order[i]])
...
def freeze_middle_sublayers(at: Atoms, planes: List[List[int]], n_freeze: int):
    """
    Congela n_freeze subcapas centrales.
    Para 12 subcapas y n_freeze=2, congela las capas 6 y 7.
    """
    n_planes = len(planes)
    if n_freeze <= 0 or n_planes == 0:
        return

    if n_freeze > n_planes:
        n_freeze = n_planes

    start = (n_planes - n_freeze) // 2
    stop  = start + n_freeze

    idx = [i for pl in planes[start:stop] for i in pl]
    if idx:
...
def enumerate_terminations_simple(
    bulk: Atoms,
    ...
    for start in range(0, len(planes_all)-n_sublayers+1):
        ...
        # aquí cambió la lógica: ya no congela las de abajo, congela las centrales
        freeze_middle_sublayers(s, planes_s, freeze_middle)
...
def kmesh_from_bulk_density(bulk_cell, bulk_k, slab_cell, clamp=(1, 64)):
    ...
    Lb = np.array([
        np.linalg.norm(bulk_cell[0]),
        np.linalg.norm(bulk_cell[1]),
        np.linalg.norm(bulk_cell[2])
    ], float)
    Lb_sorted = np.sort(Lb)[::-1][:2]

    Ls = np.array([
        np.linalg.norm(slab_cell[0]),
        np.linalg.norm(slab_cell[1])
    ], float)
...
def kmesh_from_bulk_density_using_surface_ref(
    ...
    ks1 = float(np.linalg.norm(B_slab[0]))
    ks2 = float(np.linalg.norm(B_slab[1]))
...
def frozen_indices(at: Atoms) -> set:
    ...
    for c in cons:
        if hasattr(c, "get_indices"):
            idx.update(c.get_indices())
    return idx

def write_pw_in(slab: Atoms, out_path: Path, kpts: Tuple[int,int,int], base_name: str):
...
def main():
    ...
    if not outs:
        print("[!] No QE .out files under ROOT_DIR.")
        return
    ...
    for f in outs:
        ...
        if not any(m in txt for m in END_OK_MARKERS):
            print(f"[skip] {f.name} (not converged)")
            continue
        ...
        for hkl in hkls:
            slabs = enumerate_terminations_simple(
                bulk,
                hkl,
                N_SUBLAYERS_TARGET,
                VACUUM_ANGSTROM,
            ...
            if ORIGIN_SAMPLES > 1:
                ...
                for s_idx in range(1, ORIGIN_SAMPLES):
                    ...
                    bulk_shifted = bulk.copy()
                    bulk_shifted.translate(nhat * (frac * d_hkl))
                    bulk_shifted.wrap()
    ...
    for fam_key, fmap in families.items():
        ...
        for species_key, area_key, nat, area_xy, slab, kpts in ordered:
            ...
            inpath   = DEST_DIR / "relax" / f"{base}-prerelax.in"
            lsf_path = DEST_DIR / "relax" / f"{base}-prerelax.lsf"
            jobname  = f"{base}-prerelax"

            write_pw_in(slab, inpath, kpts, base_name=base)
            write_lsf(lsf_path, inpath.name, jobname=jobname)

            print(
                f"[ok] {base}-prerelax: top={'+'.join(species_key)}; "
                f"area={area_xy:.3f} Å^2; nat={nat}; kpts={kpts}"
            )

    print("[done] One slab per (top-chemistry, area) per (system, family). "
          "12 sublayers; 2 middle sublayers frozen; kz=1.")
