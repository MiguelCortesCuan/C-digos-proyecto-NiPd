...
DEST_DIR = Path(r"G:\My Drive\Work\UNAM\Doctorado\Proyecto\Resultados\Nanoparticles\QE\Supercell\SurfaceEnergy\200_2x2\Unrelaxed")

HKL_INPUT          = (2, 0, 0)     # base Miller index; scans its {hkl} family if you want
SCAN_FAMILY        = False          # set True to scan permutations of HKL_INPUT
...
INPLANE_SUPERCELL  = (1, 1)         # e.g., set to (3,3) for 3×3
...
# k-mesh settings
BULK_KPOINTS_FALLBACK = (20, 20, 20)   # used if bulk .out has no K_POINTS automatic block
...
def enumerate_terminations_simple(bulk: Atoms, hkl: Tuple[int,int,int],
    ...
    for start in range(0, len(planes_all)-n_sublayers+1):
        ...
        # >>> in-plane supercell if requested <<<
...
# --- k-mesh alternative: orientation-agnostic Δk match ---
...
def write_pw_in(slab: Atoms, out_path: Path, kpts: Tuple[int,int,int], base_name: str):
    """
    Writes a PW input where:
      - CONTROL.title  = '<base_name>-prerelax'
      - SYSTEM.prefix  = '<base_name>'
    The file itself is expected to be named '<base_name>-prerelax.in'.
    """
...
def main():
    ...
    for f in outs:
        ...
        for hkl in hkls:
            ...
            for s in slabs:
                ...
                # Recommended k-mesh: orientation-agnostic target Δk matching
    ...
    for fam_key, fmap in families.items():
        ...
        for species_key, area_key, nat, area_xy, slab, kpts in ordered:
            ...
            # QE input + LSF script with '-prerelax' in the filename
            ...
            print(f"[ok] {base}-prerelax: top={'+'.join(species_key)}; area={area_xy:.3f} Å^2; nat={nat}; kpts={kpts}")
